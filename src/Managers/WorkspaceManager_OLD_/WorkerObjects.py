...
from src.Managers.WorkspaceManager.UserScriptsController import *
...
class jobWidget(QWidget):
    ...
    def __init__(self, processList, name, worker):
        ...
        self.jobWidgetItem.setIcon(QIcon(os.path.join(os.path.dirname(__file__), 'icons4\stop-1.png')))

    def setActive(self):
        self.active = True
        self.jobWidgetItem.setData(self.ACTIVE_FLAG, self.active)
        self.jobWidgetItem.setIcon(QIcon(os.path.join(os.path.dirname(__file__), 'icons4\hourglass-3.png')))
...
class workerObj():
    ...
    def drawSettingWindow(self):
        self.dialogBox = QDialog(self.workspace.ds)
        self.dialogBox.setModal(False)
        self.dialogBox.setMinimumWidth(300)
        self.dialogBox.setWindowTitle(self.script.name)
        self.dialogBox.setWindowIcon(QIcon(os.path.join(os.path.dirname(__file__), 'icons4\switch-4.png')))
        layout = QGridLayout()
        index = 0
