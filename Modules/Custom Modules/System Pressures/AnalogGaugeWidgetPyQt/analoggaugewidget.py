...
class AnalogGaugeWidget(QWidget):
    ...
    def create_scale_marker_values_text(self):
        ...
        if self.logMode is False:
            scale_per_div = float((self.value_max - self.value_min) / self.scala_main_count)
        else:
            scale_per_div = float((self.value_max - self.value_min) / self.scala_main_count)
        ...
        for i in range(self.scala_main_count + 1):
            ...
            if self.logMode is False:
                text = str(int(self.value_max - scale_per_div * i))
            else:
                text = 'E' + str(int(self.value_max - scale_per_div * i))
            w = fm.width(text) + 1
            h = fm.height()
